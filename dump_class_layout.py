...
def create_options():
    ...
    description = '''Dump struct layout'''
...
def find_in_modules(target, type_name):
    module_index = 0
    while True:
        module = target.GetModuleAtIndex(module_index)
        if module_index > target.GetNumModules():
            print("Type not found")
            break
        types = module.FindTypes(type_name)
        if types.GetSize():
            return ClassLayout(target, types.GetTypeAtIndex(0)).dump()
        module_index += 1

def layout(debugger, command, result, dict):
    ...
    try:
        ...
        return
    find_in_modules(debugger.GetSelectedTarget(), args.struct_name.strip('"'))

def layoutvar(debugger, command, result, dict):
    command_args = shlex.split(command)
    parser = create_options()
    try:
        args = parser.parse_args(command_args)
    ...
    if not frame.IsValid():
        print("no frame")
        return
    variable = frame.FindVariable(args.struct_name.strip('"'))
    var_type = variable.GetTypeName()
    print(var_type)
    find_in_modules(target, var_type)

def __lldb_init_module(debugger, dict):
    # This initializer is being run from LLDB in the embedded command interpreter
    # Make the options so we can generate the help text for the new LLDB
    # command line command prior to registering it with LLDB below
    debugger.HandleCommand('command script add -f dump_class_layout.layout layout')
    debugger.HandleCommand('command script add -f dump_class_layout.layoutvar layoutvar')
    print('Commands installed: "layout", "layoutvar"')